...
def open_form(recipe=None, edit=False):
    def submit_form():
...
# Function takes to form to add recipe
def add_recipe():
    open_form()
...
#edit function works similar to view and add recipe
def edit_recipe():
    selected_recipe = recipe_list.get(tk.ACTIVE)  
    if selected_recipe:
        recipe_details = recipes.get(selected_recipe)
        if recipe_details:
            open_form(recipe_details,True)
        else:
            messagebox.showerror("Error", "Recipe details not found.")
    else:
        messagebox.showerror("Error", "Please select a recipe to view.")

#function to view the recipe (one you clicked)
def view_recipe():
    selected_recipe = recipe_list.get(tk.ACTIVE) 
    if selected_recipe:
        recipe_details = recipes.get(selected_recipe)
        if recipe_details:
            open_form(recipe_details)
        else:
            messagebox.showerror("Error", "Recipe details not found.")
    else:
        messagebox.showerror("Error", "Please select a recipe to view.")
#deleting from local dictionary
def delete_recipe():
...
#create exit function to exit the app
def exit_app():
    root.destroy()
...
view_button = ttk.Button(root, text="View Recipe", command=view_recipe)
edit_button = ttk.Button(root, text="Edit Recipe", command=edit_recipe)
delete_button = ttk.Button(root, text="Delete Recipe", command=delete_recipe)
export_button = ttk.Button(root, text="Export Recipes", command=export_recipes)
